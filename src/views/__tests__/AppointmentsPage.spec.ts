...
const MockNotebookPen = {
  template: '<svg data-testid="notebook-pen-icon"></svg>',
};
...
describe('AppointmentsPage.vue', () => {
  ...
  it('deve renderizar o título principal "Meus Apontamentos" e o ícone', () => {

    const { wrapper } = mountComponent(false);

    const h1 = wrapper.find('h1');
    expect(h1.text()).toContain('Meus Agendamentos');
    expect(wrapper.find('[data-testid="notebook-pen-icon"]').exists()).toBe(true);
  });
